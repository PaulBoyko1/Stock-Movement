...
At the primary 10 bp assumed cost, the CAGR difference is approximately 1.99 percentage points, with higher annualized monthly volatility (16.83% versus 15.12%). The annualized arithmetic active mean is a different quantity: 2.08%, with an approximate interval of −1.02% to +5.18%. That interval includes zero. Full-period drawdown improved but the 2010s drawdown worsened. These findings do not establish positive factor-adjusted alpha or a deployable edge.

Decision: retain the unchanged rule for further research. Do not promote it to model-backed trading alerts. Next evaluate market/size/value/profitability/investment/momentum exposures using the same sample and HAC lag 12, then separately test an investable ETF universe with realistic execution and costs. No parameters were optimized from these results.
